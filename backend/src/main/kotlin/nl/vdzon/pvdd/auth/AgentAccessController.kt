...
@RestController
class AgentAccessController(private val access: AgentAccessVerifier, private val sessions: UserSessionService, private val config: AuthConfig) {
...
<script>document.getElementById('login').onsubmit=async(event)=>{event.preventDefault();const field=document.getElementById('token');const status=document.getElementById('status');try{const result=await fetch('/api/auth/agent-session',{method:'POST',headers:{'Content-Type':'application/json','X-AI-Access-Token':field.value},body:JSON.stringify({email:document.getElementById('email').value})});field.value='';if(!result.ok)throw new Error('Aanmelden geweigerd ('+result.status+').');const data=await result.json(); status.textContent='Je werkt nu als '+data.email;document.getElementById('open').hidden=false;}catch(error){field.value='';status.textContent=error.message;}};</script></body></html>"""
    }
}
